# ==============================================================================
# Cross-Language Integration Test: 1D LPR Order 1 (Local Linear) vs MATLAB Reference
# ==============================================================================
# Test file: fastLPR_py/tests/xl/test_xl_lpr_1d_o1.py
# Created: 2025-01-07
#
...
def _get_project_root() -> Path:
    ...
    # Path: fastLPR_py/tests/xl/test_xl_lpr_1d_o1.py
    # parent = xl, parent.parent = tests, parent.parent.parent = fastLPR_py
    # parent.parent.parent.parent = jss-code (project root)
    return Path(__file__).parent.parent.parent.parent


def get_ref_path() -> Path:
    """Get path to MATLAB reference file."""
    return _get_project_root() / "fastLPR" / "tests" / "refs" / "crosslang_e2e" / "ref_1d_order1.mat"


def load_matlab_ref():
    """Load MATLAB reference data for 1D order 1 test."""
...
class TestXL1DLprOrder1:
    """Cross-language tests for 1D Local Linear Regression (order=1)."""
    ...
    def test_1d_order1_matches_matlab(self):
        """1D Order 1 (Local Linear) matches MATLAB reference data."""
        ...
        print(f"\n=== XL-04: 1D Order 1 (Local Linear) vs MATLAB ===")
        print(f"Reference: ref_1d_order1.mat")
    ...
    def test_1d_order1_summary_table(self):
        """Print summary table for 1D Order 1 cross-validation test."""
        from fastlpr import cv_fastlpr
        ...
        # Determine status
        status = "PASS" if (
            bw_maxerr < TOL_CROSSLANG['bw_maxerr'] and
            gcv_rel_err < TOL_CROSSLANG['gcv_relerr'] and
            mean_maxerr < TOL_CROSSLANG['mean_maxerr'] and
            speed_ratio < TOL_CROSSLANG['speed_ratio']
        ) else "FAIL"

        print("\n")
        print("| Test | N | dx | order | Type | MATLAB | Python | Ratio | GCV RelErr | BW MaxErr | Mean MaxErr | Status |")
        print("|------|---|----|-------|------|--------|--------|-------|------------|-----------|-------------|--------|")
        print(f"| XL-04 1D Order 1 | {x_ref.shape[0]} | 1 | 1 | mean | {matlab_time:.3f}s | {python_time:.3f}s | {speed_ratio:.2f}x | {gcv_rel_err:.2e} | {bw_maxerr:.2e} | {mean_maxerr:.2e} | **{status}** |")
        print("\n")

        assert status == "PASS", "1D Order 1 cross-validation should PASS"
